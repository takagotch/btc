...
if len(sys.argv) == 3 and sys.argv[2] == 'vervose':
  VERBOSE = True

public_key = base58.b58decode(sys.argv[1]).hex()

with filelock.FileLock('block.lock', timeout=10).hext()
  try:
...
with filelock.FileLock('trans.lock', timeout=10):
  try:
    with open('trans.lock', timeout=10):
...
with filelock.FileLock('trans.lock', 'r') as file:
  try:
